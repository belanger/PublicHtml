...
;(defvar balance-current-line nil)

;(defface balance-current-line-face
...
;   (setq balance-current-line (make-overlay (line-beginning-position) 
;                                            (line-end-position)))
;   (overlay-put balance-current-line 'face 'balance-current-line-face)
;   (add-hook 'post-command-hook #'balance-highlight-current-line nil t)
   (hl-line-mode 1)
   (run-hooks 'balance-mode-hooks))

;(defun balance-highlight-current-line ()
;  (move-overlay balance-current-line (line-beginning-position) 
;                                     (line-end-position)))

(defun balance-current-line-number ()
  ...
  (let* ((line-beg (line-beginning-position))
    ...
    (if (string= prompt "Amount")
        (setq new-input (format (concat 
                                 ...
                                 ".2f") 
                                (string-to-number 
                                 (replace-regexp-in-string "," "" new-input))))
...
point numbers.  Balance is nil if not present."
   (let ((data-regexp "\\([-+= ]\\)[ \t]*\\([0-9.]+\\)?[ \t]*\\([-]?[0-9.]+\\)?")
      ...
      (if (match-beginning 1)
         ...
         (error "line %d, missing sign" (1+ (balance-current-line-number))))
      (if (equal sign " ")
          (progn 
            (setq sign "-")))
      (if (equal "=" sign)
      ...
      (if (match-beginning 2)
         ...
         (error "line %d, missing amount" (1+ (balance-current-line-number))))
...
(defun balance-yank-row ()
  (interactive)
  (if (<= (balance-current-line-number) 1)
